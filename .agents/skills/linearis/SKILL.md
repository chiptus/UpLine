...
- **Auth required** — any command may fail with this envelope on stderr and exit code 42:
  ...
  Detect it by `exit_code === 42` / `error === "AUTHENTICATION_REQUIRED"` (not paraphrased text). This project supplies the token via `LINEAR_API_TOKEN`, not the CLI's stored login — check presence only, never the value: `[ -n "$LINEAR_API_TOKEN" ] && echo set || echo empty`. Empty means the environment failed to deliver it (direnv not loaded locally; missing env var config in a cloud session) — tell the user `LINEAR_API_TOKEN` isn't set and let them fix delivery for that environment. Don't run `auth login` (an interactive browser flow you can't complete anyway) and don't go hunting for the token yourself. Surface `auth login`'s instruction only if the var is confirmed set and still rejected.
