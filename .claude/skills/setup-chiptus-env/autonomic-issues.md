<!-- One template, two trackers. Fill the placeholders, then delete every block
     of the tracker you didn't pick and strip the <!-- tracker:X --> / <!-- /tracker:X -->

     marker lines themselves (including this one) — the result should read as a
     plain doc with no leftover markers or unchosen-tracker text. -->
...
2. **Intake queue**: open issues labeled `needs-triage` plus open unlabeled issues<!-- tracker:github --> (`gh issue list --state open --json number,title,body,labels`)<!-- /tracker:github -->. Skip `epic` tickets. Empty queue → end silently.
