<!-- Fragment for autonomic-issues.md's Linear variant. Five sections below,
...
<!-- {{INTAKE_QUERY}}: just a period — Linear's own state already excludes
     completed/canceled issues from a default `list`, so no extra query
     needs stating inline the way GitHub's fragment states one. -->
