...
- Display formatting routes through the existing `formatInTimeZone`-based helpers in `src/lib/timeUtils.ts` (which already accepted an optional `timezone`). Day-grouping in `useScheduleData` and hour labels in the horizontal `TimeScale` also compute in the festival zone, so a post-midnight set groups by the **festival** calendar day, not the viewer's.
