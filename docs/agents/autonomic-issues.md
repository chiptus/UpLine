...
**The PR cap**: at run start the fix worker counts Linear issues labeled `agent` with status `In Review`: `npx linearis issues list --team UPL --label agent --status "In Review"`. At or above **3**, the review queue is full — end silently. Firings can overlap with no mutual exclusion, so this in-prompt count is the enforcement; a rare overshoot-by-one is accepted.
