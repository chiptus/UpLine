...
export function OrphanedSetsPanel({ orphanedSets, resolutions, onChange }: Props) {
  if (orphanedSets.length === 0) return null;

  function allArchived() {
    return orphanedSets.every((s) => (resolutions[s.id] ?? "keep") === "archive");
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-xs text-muted-foreground">
        These sets exist in the database but were not matched to any row in your CSV.
        Archived sets are hidden from users but votes are preserved.
        Default: <span className="font-medium">Keep</span>.
      </p>

      <div className="divide-y rounded-lg border">
        {orphanedSets.map((set) => {
          const resolution = resolutions[set.id] ?? "keep";
          ...
          return (
            <div key={set.id} className="flex items-center justify-between px-4 py-3">
              ...
              <div className="flex items-center gap-2 ml-4 shrink-0">
                <Label htmlFor={`orphan-${set.id}`} className="text-xs text-muted-foreground">
                  {isArchive ? "Archive" : "Keep"}
                </Label>
                <Switch
                  id={`orphan-${set.id}`}
                  checked={isArchive}
                  onCheckedChange={(checked) => onChange(set.id, checked ? "archive" : "keep")}
                />
              </div>
            </div>
          );
