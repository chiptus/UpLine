...
function useTestQueue(
  ...
  return useLinkWizardQueue(artists, untypedSets, skippedHook);
}
...
describe("useLinkWizardQueue", () => {
  ...
  it("narrows artists by selected stage and keeps currentItem consistent", async () => {
    ...
    act(() => {
      result.current.toggleStage("stage-a");
    });

    expect(result.current.items.map((i) => i.id)).toEqual(["1", "3"]);
    expect(result.current.currentItem?.id).toBe("1");
    expect(result.current.selectedStages).toEqual(["stage-a"]);

    act(() => {
      result.current.clearStages();
    });
  ...
  it("skip marks the current artist skipped and advances to the next item", async () => {
    ...
    expect(stored[EDITION_ID].records["1"].status).toBe("skipped");
  });
  ...
  it("selectItem jumps directly to the given item", async () => {
    ...
    expect(result.current.currentItem?.id).toBe("3");
  ...
  it("orders artist-less untyped set items after artist items", async () => {
    ...
    await waitFor(() => {
      expect(result.current.items.map((i) => i.id)).toEqual(["1", "set-1"]);
    });
  ...
  it("skipping a set item plainly advances without marking it in useLinkWizardSkipped", async () => {
    ...
    await waitFor(() => {
      expect(result.current.currentItem?.id).toBe("set-1");
    });
    ...
    expect(result.current.currentItem?.id).toBe("set-2");
    expect(result.current.items.map((i) => i.id)).toEqual([
      "set-1",
      "set-2",
    ]);

    expect(localStorage.getItem("link-wizard-skipped")).toBeNull();
  });
  ...
  it("filters set items by stage alongside artist items", async () => {
    ...
    expect(result.current.items.map((i) => i.id)).toEqual(["1", "set-a"]);
  });
});
