...
function skipIfNoEnv() {
  if (!SUPABASE_URL || !SERVICE_ROLE_KEY) {
    console.warn("Skipping integration tests: SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY not set");
    return true;
  }
  return false;
}
...
async function getTestEditionId(db: ReturnType<typeof adminClient>): Promise<string> {
  const { data } = await db.from("festival_editions").select("id").limit(1).single();
  assertExists(data, "No festival edition found — run test:setup first");
  return data.id;
}

async function getTestUserId(db: ReturnType<typeof adminClient>): Promise<string> {
  const { data } = await db.from("admin_roles").select("user_id").limit(1).single();
...
Deno.test("commit_schedule: updates existing set without creating duplicate", async () => {
  if (skipIfNoEnv()) return;
  const db = adminClient();
  const editionId = await getTestEditionId(db);
  const userId = await getTestUserId(db);
  ...
  const { data: artist } = await db.from("artists").select("id").eq("slug", slug).single();
  const { data: set } = await db
    .from("sets")
    .insert({ festival_edition_id: editionId, name: "Old Name", slug: "old-name", created_by: userId })
    .select("id")
    .single();
  await db.from("set_artists").insert({ set_id: set!.id, artist_id: artist!.id });
  ...
  const { data: updated } = await db.from("sets").select("name, description").eq("id", set!.id).single();
...
Deno.test("commit_schedule: archives orphaned sets", async () => {
  ...
  const { data: archived } = await db.from("sets").select("archived").eq("id", set!.id).single();
...
Deno.test("commit_schedule: midnight-crossing times stored correctly", async () => {
  if (skipIfNoEnv()) return;
  const db = adminClient();
  const editionId = await getTestEditionId(db);
  const userId = await getTestUserId(db);
