-- Add the Active scope model. See docs/adr/0005-active-group-model.md.
--
-- The original active_group_id-only model overloaded NULL to mean both
-- "never chosen" and "explicitly Everyone". Root cause: "which group is
-- mine" (active_group_id) and "which lens am I pinned to by default"
-- (active_scope) are two independent settings. active_group_id keeps its
-- original meaning; NULL now unambiguously means "no group chosen".
DO $$
BEGIN
